...
class JdbcKerugiTimerJournal(
    ...
    private var currentProjection: KerugiTimerProjection

    init { JournalSchema.migrate(dataSource); currentProjection = KerugiTimerJournal.rebuild(ownership, sessionId, events()) }
    ...
    override fun apply(command: DomainCommand, eventId: EventId): KerugiTimerResult {
        ...
        findById(eventId)?.let { existing ->
            return if (KerugiTimerJournal.sameCommand(existing.event, event)) {
    ...
    override fun events(): List<SequencedDomainEvent> = dataSource.connection.use { connection ->
        connection.prepareStatement("SELECT * FROM kerugi_timer_events WHERE bracket_id = ? AND session_id = ? ORDER BY owner_peer_id, sequence, event_id").use { statement ->
    ...
    private fun append(event: DomainEvent): SequencedDomainEvent = dataSource.connection.use { connection -> connection.inTransaction {
        ...
        connection.prepareStatement("INSERT INTO kerugi_timer_events (event_id, competition_id, owner_peer_id, court_id, bracket_id, session_id, judge_id, device_id, source, author, occurred_at, event_type, payload, sequence) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").use { statement ->
    ...
    private fun findById(eventId: EventId): SequencedDomainEvent? = dataSource.connection.use { connection ->
        connection.prepareStatement("SELECT * FROM kerugi_timer_events WHERE event_id = ?").use { statement ->
            statement.setString(1, eventId.value); statement.executeQuery().use { result -> if (result.next()) result.toSequencedEvent() else null }
        }
    }

    private fun nextSequence(connection: Connection, peerId: String): Long = connection.prepareStatement("SELECT COALESCE(MAX(sequence), 0) FROM kerugi_timer_events WHERE owner_peer_id = ?").use { statement ->
