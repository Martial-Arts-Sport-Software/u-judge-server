...
class JdbcKerugiResultJournalTest {
    ...
    @Test
    fun `rebuilds persisted golden-round result after recreation and preserves retry projection`() {
        ...
        val applied = assertIs<KerugiResultJournalResult.Applied>(first.apply(command("RED", "GOLDEN_ROUND"), eventId(10)))

        val restarted = journal(source)
        assertEquals(KerugiCompetitor.RED, restarted.projection().winner)
        assertEquals(KerugiVictoryReason.GOLDEN_ROUND, restarted.projection().reason)
        val retry = assertIs<KerugiResultJournalResult.Applied>(restarted.apply(command("RED", "GOLDEN_ROUND"), eventId(10)))
        assertEquals(false, retry.isNew)
        assertEquals(applied.event, retry.event)
        assertIs<KerugiResultJournalResult.Rejected>(restarted.apply(command("BLUE", "FINAL_SCORE"), eventId(11)))
    }
    ...
    private fun command(winner: String, reason: String) = DomainCommand(
