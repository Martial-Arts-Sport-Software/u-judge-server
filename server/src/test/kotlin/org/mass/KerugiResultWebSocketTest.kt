...
class KerugiResultWebSocketTest {
    @Test fun `authenticated result command acknowledges and publishes only a new durable decision`() = testApplication {
        ...
        senderSocket.send(Frame.Text(command(4, "BLUE", "FINAL_SCORE")))
        assertEquals("kerugi_result_ack", senderSocket.receiveJson().getValue("type").jsonPrimitive.content)
        assertUpdate(senderSocket.receiveJson(), "BLUE", "FINAL_SCORE"); assertUpdate(watcherSocket.receiveJson(), "BLUE", "FINAL_SCORE")
        senderSocket.send(Frame.Text(command(4, "BLUE", "FINAL_SCORE")))
        assertEquals("kerugi_result_ack", senderSocket.receiveJson().getValue("type").jsonPrimitive.content)
        assertEquals(null, withTimeoutOrNull(100) { watcherSocket.incoming.receive() })
        senderSocket.send(Frame.Text(command(5, "RED", "GOLDEN_ROUND")))
    ...
    private fun assertUpdate(message: kotlinx.serialization.json.JsonObject, winner: String, reason: String) { assertEquals("kerugi_result_updated", message.getValue("type").jsonPrimitive.content); assertEquals(winner, message.getValue("winner").jsonPrimitive.content); assertEquals(reason, message.getValue("reason").jsonPrimitive.content) }
}
