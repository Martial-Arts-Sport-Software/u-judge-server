...
Начата lifecycle projection: immutable `SessionProjection` валидирует переходы `prepared`, `running`, `paused`, `completed` и `cancelled`, а также rebuilds ordered history. Transport-agnostic `SessionLifecycleJournal` принимает lifecycle command только от local owner активной сетки, append-ит sequenced event до замены projection и возвращает исходный результат при повторной доставке event ID. `JdbcSessionLifecycleJournal` сохраняет полный lifecycle event envelope до изменения projection и rebuilds её после recreation из JDBC journal. Authenticated realtime sockets получают `session_state_updated` с session ID и состоянием после нового принятого lifecycle event; sender получает ACK первым, а retries и rejected commands ничего не публикуют. После нового принятого Kerugi candidate sockets также получают `kerugi_score_updated` с session ID и authoritative blue/red totals; retries и rejected score commands ничего не публикуют. Watcher authorization/UI, persistent subscription cursors, client reconnect/outbox, desktop datasource, timer, scoring и real-PostgreSQL acceptance ещё не реализованы.
...
`DiagnosticContext` даёт structured diagnostic boundary со stable IDs без персональных данных, payload или credentials.
...
`Server.start()` и desktop запускают bundled PostgreSQL 18.6 в application-data каталоге ОС, применяют миграции единого
...
pairing остаются in-memory/незавершёнными до конца I1, Kerugi handlers подключает I2. Подробности в [roadmap](docs/ROADMAP.md#инкременты-поставки).
