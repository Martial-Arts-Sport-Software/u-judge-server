...
`PostgresCommand.withAvailableLoopbackPort()` obtains an available loopback port for a new command. It does not reserve that
port across child-process launch, so `ManagedPostgres.start()` continues to diagnose an occupied port immediately before
