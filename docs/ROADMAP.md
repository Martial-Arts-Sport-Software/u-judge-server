...
Реальный входной файл получен: `/Users/maksim/Downloads/df.xlsx`. До разбора его листов и колонок `IMP-001` нельзя
считать завершённым по синтетическому формату. Нужно также подтвердить воспроизведение `PDF 1` после импорта и `PDF 2`
